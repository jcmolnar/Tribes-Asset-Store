...
function ObjectiveMission::checkScoreLimit() 
{
	...
	ObjectiveMission::refreshTeamScores();
	for(%i = 0; %i < getNumTeams(); %i++)
	if($teamScore[%i] >= $teamScoreLimit) %done = true;
		if(%done)
			ObjectiveMission::missionComplete();
}
...
//sal9k - dupe flag removal
	...
	//dupe compare to real
	%set = newObject("set",SimSet);
	%mask = $ItemObjectType;
	%num = containerBoxFillSet(%set, %mask, "0 0 0", 4000, 4000, 4000, 0);
	for(%i = 0; %i < %num; %i++)
	{
...
function Player::leaveMissionArea(%this) 
{
	...
	%location = GameBase::getPosition(Player::getClient(%this));
	alertPlayer(%this, 3, %location);
}

function alertPlayer(%player, %count, %location) 
{
	if(%player.outArea == 1) 
	{
		if(%count > 0) 
		{
			...
			schedule("alertPlayer(" @ %player @ ", " @ %count - 1 @ ", \"" @ %location @ "\"); ",1.5,%player);
		}
		else 
		{
			%set = nameToID("MissionCleanup/ObjectivesSet");
			...
			if (!$Server::TourneyMode && $Reneg::OutOfAreaDamage)
				schedule("OutOfArea(" @ %player @ ", " @ 12 @ ", \"" @ %location @ "\"); ",1.5,%player);
		}
	}
}

function OutOfArea(%player, %time, %location) 
{
	if(%player.outArea == true) 
	{
		if (!Player::isDead(%player)) 
		{
			Player::setDamageFlash(%player, 0.75);
			%amr = Player::getArmor(Player::getClient(%player));
			if(%amr == "darmor" || %amr == "harmor") 
			{
				if(%player.shieldStrength > 0)
					%amount = 0.6;
				else
					%amount = 0.3;
				GameBase::applyDamage(%player,$FlashDamageType,%amount,GameBase::getPosition(%player),"0 0 0","0 0 0",Player::getClient(%player));
			}
			else 
			{
				if(%player.shieldStrength > 0)
			...
			if(%time > 0)
				schedule("OutOfArea(" @ %player @ ", " @ %time -1 @ ", \"" @ %location @ "\"); ",3,%player);
			else 
			{
				...
				Client::sendMessage(%client,1,"You are being transported back to the mission area~waccess_denied.wav");
				GameBase::setPosition(%client, %location);
				alertPlayer(%player, 3, %location);
			}
		}
	}
}
...
{
	//echo("Initializing flag...." @ %this);
// sal9k - Flag set team pos
	$teamFlagOrigPos[Gamebase::getTeam(%this)] = GameBase::getPosition(%this);
// sal9k  - End end set team pos
...
function Flag::onDrop(%player, %type) 
{
	...
	else 
	{
		...
		if($StatTrack)
			echo("GAME: ", %playerClient, " flagdrop");
		TeamMessages(1, %flagTeam, "Your flag was dropped in the field.", -2, "", "The " @ getTeamName(%flagTeam) @ " flag was dropped in the field.");
	}
	//sal9k - ANTI JUMP
	//===var transfer
	%tmporiginalPosition = %flag.originalPosition;
	%tmpatHome = %flag.atHome;
	%tmppickupSequence = %flag.pickupSequence;
	%tmpcarrier = %flag.carrier;
	%tmpholdingTeam = %flag.holdingTeam;
	%tmpholder = %flag.holder;
	%tmpchangeTeamCount = %flag.changeTeamCount;

	%tmpenemyCaps = %flag.enemyCaps;
	%tmpcaps[0] = %flag.caps[0];
	%tmpcaps[1] = %flag.caps[1];
	%tmpcaps[2] = %flag.caps[2];
	%tmpcaps[3] = %flag.caps[3];
	%tmpcaps[4] = %flag.caps[4];
	%tmpcaps[5] = %flag.caps[5];
	%tmpcaps[6] = %flag.caps[6];
	%tmpcaps[7] = %flag.caps[7];

	deleteObject(%flag);
	%flag = newObject(Flag @ %flagTeam, Item, Flag, 1, false);
	%flag.scoreValue = 1;
	GameBase::setMapName(%flag, "Flag " @ (%flagTeam + 1));
	addToSet("MissionCleanup/ObjectivesSet", %flag);
	gamebase::setteam(%flag, %flagteam);
	$teamFlag[%flagteam] = %flag;

	%flag.originalPosition = %tmporiginalPosition;
	%flag.atHome = %tmpatHome;
	%flag.pickupSequence = %tmppickupSequence;
	%flag.carrier = %tmpcarrier;
	%flag.holdingTeam = %tmpholdingTeam;
	%flag.holder = %tmpholder;
	%flag.changeTeamCount = %tmpchangeTeamCount;

	%flag.enemyCaps = %tmpenemyCaps;
	%flag.caps[0] = %tmpcaps[0];
	%flag.caps[1] = %tmpcaps[1];
	%flag.caps[2] = %tmpcaps[2];
	%flag.caps[3] = %tmpcaps[3];
	%flag.caps[4] = %tmpcaps[4];
	%flag.caps[5] = %tmpcaps[5];
	%flag.caps[6] = %tmpcaps[6];
	%flag.caps[7] = %tmpcaps[7];
	//sal9k - END JUMP
	...
	// - Ren BW admin - flag update
	bwadmin::checkObserved(%flag, %player);
	// - End Ren BW admin - flag update
}
		...
		else 
		{
			...
			if(%flagTeam == -1) 
			{
				if(%flag.flagStand == "" || %flag.flagStand.flag != "") 
				{
					...
					Gamebase::setposition(%flag, $teamFlagOrigPos[Gamebase::getTeam(%flag)]);
			...
			ObjectiveMission::ObjectiveChanged(%flag);
			// - Ren BW admin - flag update
			bwadmin::checkObserved(%flag, %flag.carrier);
			// - End Ren BW admin - flag update
		}
	}
}
...
//	echo("Flag collision ", %object);
	if(Player::getMountObject(%object) != -1)
		return;
	...
	if(%flagTeam == %playerTeam) 
	{
		if(!%this.atHome) 
		{
			...
			Gamebase::setposition(%this, $teamFlagOrigPos[Gamebase::getTeam(%this)]);
			...
			if($StatTrack)
				echo ("GAME: ", %playerClient, " flagreturn");
		...
		else 
		{
			if(%object.carryFlag != "") 
			{
				...
				if(%enemyTeam != -1) 
				{
					...
					// - Ren BW admin - flag update
					bwadmin::checkObserved(%flag, %flag.carrier);
					// - End BW admin - flag update
					%playerClient.score += 5;
					Game::refreshClientScore(%playerClient);
					if($StatTrack)
					echo ("GAME: ", %playerClient, " flagcapture");
	...
	else 
	{
		if(%object.driver)
			return;
		if(getNumClients() < 3 && !%playerClient.isSuperAdmin)
		{
			if(getNumClients() < 2) 
			{
				centerprintall("<jc><f2>You can't capture an undefended flag\n\nHave you noticed that you're all alone?", 2);
				return;
			}
		else 
		{
			%pl = Client::getFirst();
			%plb = Client::getNext(%pl);
			if(%pl.observerMode != "" || %plb.observerMode != "" || GameBase::getTeam(%pl) == GameBase::getTeam(%plb)) 
			{
				centerprintall("<jc><f2>A player on each team\nis required to capture the flag", 3);
				return;
			}
		}
	}
	if(%object.carryFlag == "") 
	{
		if(%object.outArea == "") 
		{
			...
			// - Ren BW admin - flag update
			bwadmin::checkObserved(%this, %this.carrier);
			// - End BW admin - flag update
			...
			if(%flagTeam != -1) 
			{
				...
				if($StatTrack)
					echo ("GAME: ", %playerClient, " flaggrab");
...
function Flag::setWaypoint(%client, %flag) 
{
	...
	{
		%pos = $teamFlagOrigPos[Client::getTeam(%client)];
	...
	if(%this.carrier == %playerId) 
	{
		...
		else 
		{
			...
			else 
			{
				...
				Item::setVelocity(%this, "0 0 0");
				// Patch
				%this.currentFlagStand = %this.flagStand;
			}
...
function StaticShape::objectiveDestroyed(%this) 
{
	if($StatTrack) 
	{
		...
		echo("GAME: ",%playerclient," Destroyed ", %name.className);
	}
	if(%this.destroyed == "") 
	{
		if(!%this.objectiveLine)
