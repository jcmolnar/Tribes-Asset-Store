...
function beginMatchMode()
{
	...
	if ($Shifter::DetPackLimit == ""){ $Shifter::DetPackLimit = "15"; }
		...
		$TeamItemMax[MFGLAmmo] = $Shifter::NukeLimit;
		$Server::Info = $Server::Info @ "\nRunning Shifter_v1G " @ $greyflcn::newdate;
		// PORT NOTE (2026-09-12): was "Shifter_v1G " @ $greyflcn::newdate -- a slip, since
		// the three sibling assignments (admin.cs:10, 2108, 2193) all set the bare word.
		// $ModList is what the server ADVERTISES, and the Modern Client matches a join
		// against it as an exact word set: advertising "Shifter_v1G 5-27-2002" makes the
		// mod-join prompt miss and the player joins as plain base. It also puts a bogus
		// "5-27-2002" block on the search path the next time EvalSearchPath runs.
		if(!$noTabChange) $ModList = "Shifter_v1G";
		$Server::MODInfo = $Server::MODInfo @ "\nRunning Shifter_v1G " @ $greyflcn::newdate;
	  	 Server::storeData();
