...
// - BW Admin Mod -
		   ...
		   if($Server::TourneyMode && $TAC::controlledTourneyMode && $TAC::reportScores)
		      messageAll(0, "Final Score: " @ getTeamName(%i) @ ": " @ $teamScore[%i]);
		}
// - BW Admin Mod - End
...
// - BW Admin Mod -
		   ...
		   {
		   	if(!%tieGame)
		     	  messageAll(0, getTeamName(%curLeader) @ " won the match!");
		     	else
		     	  messageAll(0, "The match was a draw!");
		   }
...
// - BW Admin Mod -
			if($teamScoreLimit == "false")
				Team::setObjective(%i, 2,"<f1>   - No score limit on the game.");				else
// - BW Admin Mod - End
...
function ObjectiveMission::checkScoreLimit()
{
   %done = false;
   ObjectiveMission::refreshTeamScores();
// - BW Admin Mod - to disable score limit
   if($teamScoreLimit == "false")
      return;
// - BW Admin Mod - End
...
function ObjectiveMission::initCheck(%object)
{
	if($TestMissionType == "")
	{
		%testmissiondata = gettacmissiontype($missionName);
		if(%testmissiondata == "TDM")
			$TestMissionType = "TDM";
		else if(%testmissiondata == "CTF")
			$TestMissionType = "CTF";
		else if(%testmissiondata == "C&H")
			$TestMissionType = "C&H";
		else if(%testmissiondata == "D&D")
			$TestMissionType = "D&D";
		else if(%testmissiondata == "F&R")
			$TestMissionType = "F&R";
	}
...
function gettacmissiontype(%mission)
{
	%x = 0;
	%i = 0;
	while(%i<BWStrLen(%mission))
	{
		%char = String::getSubStr(%mission,%i,1);
		if(!String::ICompare(%char, "_"))
		{
			%left = String::getSubStr(%mission,0,%i);
			%right = String::getSubStr(%mission,%i+1,(BWStrLen(%mission)-%i));
			%missionspaced = strcat(%left," ",%right);
			%x++;
		}
		%i++;
	}
	%tacmissiontype = getWord(%missionspaced, %x);
	if(%tacmissiontype == -1)
		%tacmissiontype = getWord(%missionspaced, 1);
	return %tacmissiontype;
}
   ...
   if($dedicated && $matchStarted && $TAC::controlledTourneyMode && $Server::TourneyMode && $TAC::messageLog)
      {
         ...
         echo("TACMSG (" @ %time @ "): " @ %clientName @ " received 5 points for holding a captured tower.");
      }
// - BW Admin Mod - End
...
// - TAC Mod - Update those spectating the flag
   TAC::checkObserved(%flag, %player);
// - TAC Mod - End
}
...
function Flag::checkReturn(%flag, %sequenceNum)
{
   ...
   if(%flag.pickupSequence == %sequenceNum && %flag.timerOn == "")
   {
		...
		else
		{
			...
			ObjectiveMission::ObjectiveChanged(%flag);
// - TAC Mod - Update those spectating the flag
			TAC::checkObserved(%flag, %flag.carrier);
		}
	}
}
...
// - BW Admin Mod - End
			   ...
			   $teamScore[%playerTeam] += $TAC::FlagValue;
				...
				ObjectiveMission::checkScoreLimit();
				%teamcounter = 0;
				TeamScoreIncrease(%playerClient, %playerTeam, %flag);
	    		  messageAll(0, Client::getName(%playerClient) @ "'s team receives " @ $TAC::FlagValue @ " point capture bonus.");
// - TAC Mod - Update those spectating the flag
               TAC::checkObserved(%flag, %object);
            }
         }
      }
   }
   else
   {
      ...
      if((%object.carryFlag == "") && (%object.driver != 1))
      {
         if(%object.outArea == "")
         {
         // don't pick up our flags
            ...
            if($TAC::walk)
               Player::mountItem(%object, Flag, $WeaponSlot, %flagTeam);
            else
            ...
            if($TAC::walk)
               TAC::walk::energysuck(%object);

// - TAC Mod - Update those spectating the flag
            TAC::checkObserved(%this, %object);
// - TAC Mod - End

      	    Flag::setWaypoint(%playerClient, %this);
      	    %playerClient.flagnumber = %this;
            if(%this.fadeOut)
      	    {
      	       GameBase::startFadeIn(%this);
               %this.fadeOut= "";
...
//==================Code to increase all members score by 5 points==========
function TeamScoreIncrease(%clientId, %TeamNumber, %flag)
{
	for(%cl = Client::getFirst(); %cl != -1; %cl = Client::getNext(%cl))
   	{
		if (Client::getTeam(%clientId) == %TeamNumber)
		{
			%clientId.score += $TAC::FlagValue;
			%pilot = %flag.lastpilot;
			if(%pilot > 0)
			{
				%pilot.score += 3;
				%flag.lastpilot = 0;
...
// - TAC mod - updating those spectating the flag
			TAC::checkObserved(%flag, %flag.carrier);
...
// - TAC Mod - Update those spectating the flag
      TAC::checkObserved(%this, %playerId);
// - TAC Mod - End
