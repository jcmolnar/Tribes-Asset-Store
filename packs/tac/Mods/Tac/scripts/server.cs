...
{
	//remove players ownership of vehicle
	...
	TAC_ResetTKCount(%clientId);

	%clientId.TACVoiceSpamCount = 0;
	%clientId.VoicePackSpamTime = 0;
	%clientId.TACSpamOffence = 0;
	%clientId.lastknockerattacker = 0;
	%clientId.lastknockerattack = 0;
	%clientId.PilotView = "";
...
function Server::onClientConnect(%clientId)
{
	%ip = Client::getTransportAddress(%clientId);
   	if(!String::NCompare(Client::getTransportAddress(%clientId), "LOOPBACK", 8))
   	{
      	...
      	%clientId.UserLevel = "SuperAdmin";
	  	TACSetClientAdmin(%clientId);
   	}
   	else
   	  	TAC_Authenticate(%clientId, %ip);
...
function createServer(%mission, %dedicated)
{
   ...
   $ME::Loaded = false;
	//------Overwrite lastmission settings and use mission specified in TAC.cs-------
	//   if(%mission == "")
      //  	  %mission = $pref::lastMission;

	if(%mission == "")
      	%mission = $TAC::DefaultServerMap;

	if(%mission == "")
      	%mission = "Air_Coaster_CTF";
...
function Server::nextMission(%replay)
{
   ...
   else if ($TAC::MRenabled == "true")
		%nextMission = MR::nextMission ();
   else
...
// - TAC - Pass the Objectives to clients
...
// - BW Admin Mod - Victory screen displayed for longer & team names/skins swapped if set
   if($TAC::teamSwap)
   {
      %oldteamName[0] = $Server::teamName[0];
      %oldteamSkin[0] = $Server::teamSkin[0];
      $Server::teamName[0] = $Server::teamName[1];
      $Server::teamSkin[0] = $Server::teamSkin[1];
      $Server::teamName[1] = %oldteamName[0];
      $Server::teamSkin[1] = %oldteamSkin[0];
      $TAC::teamSwap = "";
      messageAll(0, "TAC:: Team names and skins swapped!");
   }
// - BW Admin Mod - Synchronise
	if($TAC::KeepSmurfLog == "true")
	  	BackupSmurfRecord();
   ...
   if($Server::TourneyMode)
      $TAC::AutoAntiTK = "";
   if($Server::TourneyMode && !%immed)
   {
      schedule("Server::finishMissionLoad();", 30);
      return;
   }
   else if(!%immed)
...
function Server::finishMissionLoad()
{
   ...
   //ensure smurf hunter timer continues after mission change
   if($TAC::KeepSmurfLog == "true")
...
//== Team Centreprint added== Wizard_TPG
function centerprintteam(%msg, %timeout, %teamnumber)
{
   if(%timeout == "")
      %timeout = 5;
   for(%clientId = Client::getFirst(); %clientId != -1; %clientId = Client::getNext(%clientId))
	if (GameBase::getTeam(%clientId) == %teamnumber)
      	remoteEval(%clientId, "CP", %msg, %timeout);
}

function bottomprintteam(%msg, %timeout, %teamnumber)
{
   if(%timeout == "")
      %timeout = 5;
   for(%clientId = Client::getFirst(); %clientId != -1; %clientId = Client::getNext(%clientId))
...
function topprintteam(%msg, %timeout, %teamnumber)
{
   if(%timeout == "")
      %timeout = 5;
   for(%clientId = Client::getFirst(); %clientId != -1; %clientId = Client::getNext(%clientId))
...
//== Team Centreprint added== Wizard_TPG == END