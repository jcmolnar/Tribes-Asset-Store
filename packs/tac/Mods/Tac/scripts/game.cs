...
// - BW Admin Mod -
if($TAC::DefaultTeamEnergy != "")
  $DefaultTeamEnergy = $TAC::DefaultTeamEnergy;
else
// - BW Admin Mod - End
...
//======Ground Kill Messages==============

$deathMsg[-3,0]						 = "%1 scored a touchdown - not such a great idea.";
$deathMsg[-3,1]						 = "%1 touched %2 feet on the ground.";
$deathMsg[-3,2]						 = "%1 was glad to get %2 feet back on solid ground, for a little while.";
$deathMsg[-3,3]						 = "%1 exploded from touching the ground.";
$deathMsg[-3,4]						 = "%1 just blew %2 ass off.";
$deathMsg[-3,5]						 = "%1 found the instructions didnt lie.";
$deathMsg[-3,6]						 = "%1's feet really hurt.";
$deathMsg[-3,7]						 = "%1 bought the farm.";


$numDeathMsgs = 4;
$numGroundDeathMsgs = 8;
//---------------------------------------------------------------------------------
...
// - BW Admin Mod - Advanced console logging for stats tracking programs
      if($dedicated && $TAC::controlledTourneyMode && $Server::TourneyMode && $TAC::messageLog)
      {
         %name = Client::getName(%cl);
         %teamName = getTeamName(Client::getTeam(%cl));
         echo("TACMSG (0): " @ %name @ " joined " @ %teamName);
      }
// - BW Admin Mod - End
...
function Game::playerSpawn(%clientId, %respawn)
{
   ...
   if(!%respawn)
   {
      // initial drop
// - BW Admin Mod -
      bottomprint(%clientId, "<jc><f0>This server is running: <f1>Team Aerial Combat " @ $TAC::version @ "\n\n<f0>Mission: <f1>" @ $missionName @ "   <f0>Mission Type: <f1>" @ $TestMissionType @ "\n\n<f0>URL: <f1>http://www.tribes-universe.com/tac/" , 5);
   }
// - BW Admin Mod - End
	if(%spawnMarker) {
		%clientId.guiLock = "";
...
function Game::initialMissionDrop(%clientId)
{
   ...
   {
// - BW Admin Mod - New Observer Mode
...
// - BW Admin Mod -
         if($TAC::controlledTourneyMode)
            bottomprint(%clientId, "<jc><f0>This server is running Team Aerial Combat (TAC) Mod " @ $TAC::version @ ".\n\n<f1>" @ $TAC::league @ " Match in progress, press TAB for options.", 8);
         else
            bottomprint(%clientId, "<jc><f0>This server is running Team Aerial Combat (TAC) Mod " @ $TAC::version @ ".\n\n<f1>Match in progress, press TAB for options.", 8);
	...
	if($TAC::controlledTourneyMode)
	{
		...
		{
		   if($Client::info[%clientId, 3] == "[press]")
	           {
	        	processMenuInitialPickTeam(%clientId, -2);
		        bottomprint(%clientId, "<jc><f0>This server is running Team Aerial Combat (TAC) Mod " @ $TAC::version @ ".\n\n<f1>Welcome to the " @ $TAC::league @ ".\nLeague Match Mode engaged - You were auto-assigned to observer!", 5);
			%clientId.justConnected = "";
			return;
		   }
		   if($Client::info[%clientId, 3] == getTeamName(%i))
	           {
	        	processMenuInitialPickTeam(%clientId, %i);
		        bottomprint(%clientId, "<jc><f0>This server is running Team Aerial Combat (TAC) Mod " @ $TAC::version @ ".\n\n<f1>Welcome to the " @ $TAC::league @ ".\nLeague Match Mode engaged - You were auto-assigned to the " @ getTeamName(%i) @ " team!", 5);
			%clientId.justConnected = "";
			return;
		   }
	        }
	        bottomprint(%clientId, "<jc><f0>This server is running Team Aerial Combat (TAC) Mod " @ $TAC::version @ ".\n\n<f1>Welcome to the " @ $TAC::league @ ".\nLeague Match Mode engaged - Choose your tribe!", 0);
	}
	else
	   bottomprint(%clientId, "<jc><f0>This server is running Team Aerial Combat (TAC) Mod " @ $TAC::version @ ".\n\n<f1>Competition Mode engaged - Choose your tribe!", 0);
// - BW Admin Mod - End
      }
...
{
// - BW Admin Mod - Don't check if in controlled mode
...
{
	if(%playerid.lastknockerattacker >= 2000)
	{
		%killerId = %playerid.lastknockerattacker;
		%damageType = %playerid.lastknockerattack;
		%playerId.lastknockerattacker = 0;
		%playerId.lastknockerattack = 0;
	}
	...
	// - TAC Mod - anti-TK Code
		   //%killerId.tk++;
		   //schedule(%killerId @ ".tk--;", 180, %killerId);
		   if($TAC::AutoAntiTK)
				TAC_setTeamKill(%playerId, %killerId);
   	...
   	{
		//========================Play random message for Ground Kill====================
		%victimName = Client::getName(%playerId);
		if(%victimName == "")
		{
			%playerId = $LastPlayerKilled;
			%victimName = Client::getName(%playerId);
		}
		echo("GAME: " @ %playerId @ " was killed from touching the ground.");
		if(!String::ICompare(Client::getGender(%playerId), "Male"))
			%playerGender = "his";
		else
			%playerGender = "her";
		%ridx = floor(getRandom() * ($numGroundDeathMsgs - 0.01));

		%oopsMsg = sprintf($deathMsg[-3, %ridx], %victimName, %playerGender);
		messageAll(0, %oopsMsg, $DeathMessageMask);
	}
}
