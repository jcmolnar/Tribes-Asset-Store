...
{
	if ((getObjectType(%object) == "Player") && (%object.driver != 1))
	{
		%item = Item::getItemData(%this);
...
// - TAC Mod - Long Walk Home support
function Weapon::onUse(%player,%item)
{
	if(%player.Station=="" && (!$TAC::walk || %player.carryflag ==""))
	{
...
function RepairPack::onUse(%player,%item)
{
	...
	else if(!$TAC::walk || %player.carryflag =="")
	{
...
function ShieldPackImage::onDeactivate(%player,%imageSlot)
{
	...
	%player.shieldcheck = 0;
	schedule("GroundKillCheck("@%player@",true, 0, true);",0.1,%player);
