...
{
   baseVolume = 0;
   // Idle loop of 8 vehicle datablocks (SoundDiscSpin) plus SoundFlyerIdle, so a hosted
   // map stacks them. TWO separate causes, fixed in two passes:
   //  1. minDistance was 50 -- the reference distance, inside which NO attenuation is
   //     applied at all, so every parked aircraft was at FULL volume within 50 units.
   //     Now 2.0, matching Profile3dNearLoop/Profile3dMediumLoop.
   //  2. maxDistance was 800, and that alone kept it deafening (Joe, still loud after
   //     pass 1). soundFXBuffer.cpp:815-823 ramps LINEARLY -- factor =
   //     (dist-min)/(max-min) -- and that factor becomes DECIBELS at :765,
   //     SetVolume(volume * -10000). Linear-in-dB over 800 units means 50 units away is
   //     only -6 dB and 100 units only -12 dB, i.e. barely quieter anywhere you can
   //     stand. Base's Profile3dNearLoop (max 40) is -47 dB by 20 units. 200 keeps this
   //     profile's "carries further" intent (2x base Profile3dMediumLoop's 100) while
   //     giving -24 dB at 50 and silence by 200.
   //     NOTE the sqrt shaping at soundFXBuffer.cpp:821 is commented out in the engine,
   //     so there is no curve to soften this -- maxDistance IS the whole falloff.
   minDistance = 2.0;
   maxDistance = 200.0;
   flags = { SFX_IS_HARDWARE_3D, SFX_IS_LOOPING };
};
