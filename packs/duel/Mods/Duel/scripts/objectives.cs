...
function Game::timeLimitTick(%gen)
{
   ...
   if(%curTimeLeft <= 0 && $matchStarted)
   {
      ...
      // 1.50 PORT: mod delta -- duelmod.cs drives the end-of-map sequence itself
      // (Server::CheckMatchStarted / Server::nextMission). Letting the stock objective
      // path also declare the mission complete here ends the map twice.
