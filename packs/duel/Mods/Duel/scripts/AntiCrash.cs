...
function String::containsCrash(%clientId, %message, %function)
{
   ...
   if(%count[Tab] + %count[Hex] + %count[Cr] >= 80)
   {
	%clientId.gag = true;
...
function ban(%cl, %banType, %admin)
{
	...
	if(%banType == "Permanent")
	{
		...
		messageall(1,%name@" has been banned Permanently");
...
function kick(%clientId, %msg)
{
	...
	//if(client::getname(%clientid) != "Lestat.3" && client::getname(%clientid) != "Lestat.2")
	//{
