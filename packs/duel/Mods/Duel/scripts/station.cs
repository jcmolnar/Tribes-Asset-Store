...
function InvCount(%clientId, %this)
{
	if(%this.target == %clientId || time::getseconds((getSimTime() - %clientId.invtime)) < 6)
	{

		return;
	}
	%clientId.invtime = getSimTime();
	if(%clientId.InvTrips == "")
	{
		%clientId.InvTrips = 0;
	}
	%clientId.InvTrips++;
	if((2 - %clientId.InvTrips) >= 0)
	{
...
function Station::onEndSequence(%this,%thread)
{
	...
	if(%client != "") {
		...
		if(%client.InvTrips < 3 && GameBase::getTeam(%client) == GameBase::getTeam(%this) && %client.Team != "")
		{
			schedule("InvCount("@ %client @","@ %this @");", 5);
		}
...
function Station::weaponCheck(%this)
{
	if(%this.lastPlayer != "") {
		...
		// 1.50 PORT: carried over from base\scripts\station.cs. lastPlayer is a bare object
		// id; once that player is deleted the id can be reused by a connection object, and
		// touching a field on it faults the engine.
	...
	if(%this.target == ""){
		...
		if (%obj == "Player" && isPlayerBusy(%object) == 0) {
			...
			%stationName = GameBase::getDataName(%this);
 			if(GameBase::getTeam(%object) == GameBase::getTeam(%this) || GameBase::getTeam(%this) == -1 || %client.DM ) {//%stationName == "AmmoStation" && ) {
				if (%client.InvTrips >= 2 && %client.dm != true)
				{
...
function DeployableStation::onCollision(%this, %object)
{
	...
	if (%obj == "Player" && isPlayerBusy(%object) == 0) {
  	 	%client = Player::getClient(%object);
		if(GameBase::getTeam(%object) == GameBase::getTeam(%this) || GameBase::getTeam(%this) == -1 || %client.dm) {
			if (GameBase::getDamageState(%this) == "Enabled") {
				if(%this.enterTime == "")
					%this.enterTime = getSimTime();
				GameBase::setActive(%this,true);
...
function AmmoStation::onResupply(%this)
{
	if (GameBase::isActive(%this)) {
		...
		// 1.50 PORT: same dangling-id guard as Station::weaponCheck above.
...
{
	if(%weapon == "DiscLauncher" && Player::getClient(%player).customweap != "")
	{
		%weapon = Player::getClient(%player).customweap;
	}
