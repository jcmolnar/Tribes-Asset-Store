...
function Torture::OnAdd(%this)
{
	Torture::Loop(%this);
}

function Torture::Loop(%this)
{
	both(GameBase::getDataName(%this) @" "@GameBase::getControlClient(%this));
	//return;
	if(GameBase::getDataName(%this) == "Torture")
	{

		if(GameBase::getControlClient(%this) != -1)
		{
			both("its controlled... toning down vel...");
			//item::setvelocity(%this, vector::add(item::getvelocity(%this),"0 0 -1000"));
			//schedule("Torture::Loop("@%this@");",1);

			//resetlos();
			//gamebase::getlosinfo(%this, 500, "-1.57 0 0");
			//gamebase::setposition(%this, $los::position);
			//resetlos();
			//gamebase::getlosinfo(GameBase::getControlClient(%this), 500, "-1.57 0 0");
			//gamebase::setposition(GameBase::getControlClient(%this), $los::position);
			//resetlos();
		}
		else {
			both("no controller... resume loop");
			//schedule("Torture::Loop("@%this@");",1);
		}
	}
	schedule("Torture::Loop("@%this@");",1);
