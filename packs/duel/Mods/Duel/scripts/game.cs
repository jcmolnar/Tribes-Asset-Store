...
$spawnBuyList[3] = DiscLauncher;   // 1.50 PORT: mod delta -- upstream casing fix
...
{
   // 1.50 PORT: mod delta -- the duel mod lets OBSERVERS open the menu (that is how you
   // join a duel queue while dead/spectating), so the observer test is deliberately gone.
   if(!%clientId.guiLock)
   {
      remoteSCOM(%clientId, -1);
...
function Game::initialMissionDrop(%clientId)
{
   ...
   else
   {
      ...
      // 1.50 PORT: mod delta -- a duel server never auto-assigns teams; TDMenu/TDMain own
      // team placement (challenge, ladder and arena queues). Auto-assign here would fight them.
