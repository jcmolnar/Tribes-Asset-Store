...
$ArmorKickback[iarmorSpy] = 1.20;

PlayerData armormSpy
{
	className = "Armor";
	shapeFile = "larmor";
...
function armormSpy::onPoison(%client, %player)
{
	Armor::onPoison(%client, %player);
}

function armormSpy::onBurn(%client, %player)
{
	Armor::onBurn(%client, %player);
}

function armormSpy::onShock(%client, %player)
{
	Armor::onShock(%client, %player);
}


function armormSpy::onPlayerContact(%targetPlayer, %sourcePlayer)
{		
	Assassinate(%targetPlayer, %sourcePlayer);
}

function armormSpy::onGrenade(%player)
{
...
function armormSpy::onBeacon(%player, %item)
{
...
function armorfSpy::onPoison(%client, %player)
{
	Armor::onPoison(%client, %player);
}

function armorfSpy::onBurn(%client, %player)
{
	Armor::onBurn(%client, %player);
}

function armorfSpy::onShock(%client, %player)
{
	Armor::onShock(%client, %player);
}


function armorfSpy::onPlayerContact(%targetPlayer, %sourcePlayer)
{
	Assassinate(%targetPlayer, %sourcePlayer);
}

function armorfSpy::onGrenade(%player)
{
	Client::sendMessage(Player::getClient(%player),1, "Plastique Explosive will explode in 15 seconds");
	%obj = newObject("","Mine","Nukebomb");
	Armor::ThrowGrenade(%player, %obj);
...
function armorfSpy::onBeacon(%player, %item)
{
	//startCloak(Player::getClient(%player));
	if(!$build)
		Annihilation::decItemCount(%player, %item);
...
Armor::add("Spy", "Chameleon Assassin", 175);	//  Name, description, price