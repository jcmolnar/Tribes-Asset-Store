...
//	   Item::setVelocity(%client, "0 0 0");	//Stop the client, so inherited velocity doesn't affect JetToPos results.
   ...
   else
   {
	...
	if(%targetPos == "break"){return False;break;}
...
function WarpMyAss(%client, %targetPos)
{
	...
	//  messageall(1, "Warping the ass of bot.");
		GameBase::setPosition(%client,%targetPos);
		$Spoonbot::BotJettingHeat[%client]=0;
//		Item::setVelocity(%client, "0 0 0");
		$JetToPos[%client]="break";
	...
	$Server::teamName[0] = "Autobots";		// Team 1 Name
	
			%switch = floor(getRandom() * 3);
			if(%switch == 0)
			{
				$Server::teamSkin[0] = "cphoenix";
			}
			else if(%switch == 1)
			{
				$Server::teamSkin[0] = "blue";
			}
				else if(%switch == 2)
			{
				$Server::teamSkin[0] = "orange";
			}
if($TALT::Active)
	$Server::teamName[1] = "DS";
else
	$Server::teamName[1] = "Androids";	// Team 2 Name

			%switch = floor(getRandom() * 3);
			if(%switch == 0)
			{
				$Server::teamSkin[1] = "swolf";
			}
			else if(%switch == 1)
			{
				$Server::teamSkin[1] = "green";
			}
				else if(%switch == 2)
			{
			...
			$ANNIHILATION::UsePersonalSkin = 0;
			
 $Server::teamName[2] = "Children of the Phoenix";	// Team 3 Name
 $Server::teamSkin[2] = "beagle";		// Team 3 Skin
 $Server::teamName[3] = "Starwolf ";		// Team 4 Name
 $Server::teamSkin[3] = "dsword";			// Team 4 Skin
