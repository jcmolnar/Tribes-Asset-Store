...
$ArmorKickback[iarmorTank] = 0.80;

PlayerData armorTank 
{
	//shieldStrength = 0.5;	//0.024;
	className = "Armor";
	shapeFile = "harmor";
	...
	jetEnergyDrain = 1.3;
	maxDamage = 1.8;	//1.9;
	maxForwardSpeed = 10.75;
	maxBackwardSpeed = 10.5;
	maxSideSpeed = 8.5;
	groundForce = 35 * 18.0;
	mass = 20.0;
	groundTraction = 4.5;
...
function armorTank::onPoison(%client, %player)
{
	// Can't poison
}

function armorTank::onBurn(%client, %player)
{
	Armor::onBurn(%client, %player);
}

function armorTank::onShock(%client, %player)
{
	//Armor::onShock(%client, %player);
}


function armorTank::onPlayerContact(%damagedPlayer, %damagingPlayer)
{
	Drain(%damagedPlayer, %damagingPlayer);
}

function armorTank::onGrenade(%player)
{
	%obj = newObject("","Mine","TankBomb");
	Armor::ThrowGrenade(%player, %obj);
}

function armorTank::onBeacon(%player, %item)
{
	//Armor::TankSpeedBooster(%player, %item, 450);	
...
//		%obj = Projectile::spawnProjectile("TankShockShell", %trans, %player, %vel);
//		Projectile::spawnProjectile(%obj);		
//		%player.usedbeacon = true;
//		schedule(%player@".usedbeacon = false;",2.5);
//	}
//}
...
Armor::add("Tank", "Tank", 500);	//  Name, description, price