...
{
	DebugFun("Elevator::onAdd",%this);
	%this.inmotion = true;
	%this.delayTime = getSimTime();
	// schedule("Elevator::check("@%this@");",0.1,%this);
}

function Elevator::check(%this)
{
	DebugFun("Elevator::check",%this);
	%set = newObject("set",SimSet);
	%num = containerBoxFillSet(%set,$MineObjectType,GameBase::getPosition(%this),6,6,10,0);
	%totalnum = Group::objectCount(%set);
	for(%i = 0; %i < %totalnum; %i++)
	{
		%obj = Group::getObject(%set, %i);
		if ( getObjectType(%obj) == Mine )
		{
	...
	deleteObject(%set);
	schedule("Elevator::check("@%this@");",0.01,%this);
}

function Elevator::onNewPath(%this)
{
	DebugFun("Elevator::onNewPath",%this);
	%this.status = "up";
...
{
	DebugFun("Elevator::onEnabled",%this);
...
{
	DebugFun("Elevator::trigger",%this);
...
{
	DebugFun("Elevator::onPower",%this,%state,%generator);
	if(%state)
	{
		if(Moveable::getPosition(%this) != 0 || %this.loop != "")
		{
...
{
	DebugFun("Elevator::onFirst",%this);
	%this.status = "up";
...
{
	DebugFun("Elevator::onWaypoint",%this);
...
{
	GameBase::applyDamage(%obj,$CrushDamageType,0.01,GameBase::getPosition(%this),"0 0 0","0 0 0",%this);
}

function Elevator::onDamage(%this,%type,%value,%pos,%vec,%mom,%object)
{
	DebugFun("Elevator::onDamage",%this,%type,%value,%pos,%vec,%mom,%object);
	if($debug::Damage)
	{
		Anni::Echo("Elevator::onDamage("@%this@", "@%type@", "@%value@", "@%pos@", "@%vec@", "@%mom@", "@%object@" )");
	}	
}

function Elevator::onDisabled(%this)
{
	DebugFun("Elevator::onDisabled",%this);
...
{
	DebugFun("Elevator::onDestroyed",%this);
	GameBase::setActive(%this,false); 		
	Moveable::stop(%this);
}	
...
{
	DebugFun("Elevator::checkDelay",%this);
	if(getSimTime() >= %this.delayTime) 
		Elevator::onMove(%this);
	else
...
{
	DebugFun("Elevator::onTrigEnter",%this,%object,%trigger);
}

function Elevator::onTrigLeave(%this,%object,%trigger)
{
	DebugFun("Elevator::onTrigLeave",%this,%object,%trigger);
}

function Elevator::onTrigger(%this,%object,%trigger)
{
	DebugFun("Elevator::onTrigger",%this,%object,%trigger);
...
function Door::onDamage(%this,%type,%value,%pos,%vec,%mom,%object)
{
	if($debug::Damage)
	{
		Anni::Echo("Door::onDamage("@%this@", "@%type@", "@%value@", "@%pos@", "@%vec@", "@%mom@", "@%object@" )");
	}
