...
exec("editor\\AnnEditor.cs");

function saveMission( %objName, %fileName )
{
	Anni::Echo("savemission");
	Anni::Echo( %objName, %fileName );
	focusServer();
...
{
	Anni::Echo("missionload object ");
	Anni::Echo( %objParentName, %fileName );
	focusServer();
...
function MEMode()
{
	...
	//MissionObjectList::AddDisplayGroup(1, "CommandShip0");
...
{		
	...
	{	
		$Annihilation::ExtendedInvs = "";	//disabling additional triggers if the editor is open...
		messageall(0,"Opening Mission Editor, Extended Inventories disabled.~wfemale1.wdsgst4.wav");
	//	if(!$debug)	
...
{
	Anni::Echo("cam to player");
	MissionObjectList::onSelectionCleared();	//deselect whatever is current so it doesnt get deleted
	  focusServer();			// let the server do its thing	
	%cl = Client::getFirst();		// first client is editing, unless someone joined during start up
	%player = Client::getOwnedObject(%cl);
	%pos = vector::add(GameBase::getPosition(%player),"0 0 2.5");	//place object above player	
	%mSensor = newObject("","Sensor",DeployableMotionSensor,true);
	addToSet("MissionCleanup", %mSensor);
	GameBase::setPosition(%mSensor,%pos);				
	  focusClient();			// back to player to allow editing
	$ME::InspectObject = %mSensor;	
	ME::CameraToObject();			//set cam at new object
	MissionObjectList::onSelected(1, %mSensor);	//select new object
	ME::DeleteSelection();			//think about it..
}
