...
function showTime()
{
	function Game::EndFrame()
	{
		Anni::Echo("C: " @ getIntegerTime(false) @ "	S: " @ getIntegerTime(true));
	}
...
function showFPS()
{
	function Game::EndFrame()
	{
		Anni::Echo($ConsoleWorld::FrameRate);
	}
	$Console::LastLineTimeout = 1000;
	$Console::updateMetrics = false;
}
...
function showGfxOGL()
{
	function Game::EndFrame()
	{
		Anni::Echo($ConsoleWorld::FrameRate @ " T: " @ $OpenGL::TexDL @ " L: " @ $OpenGL::LMDL @ " LB: " @ $OpenGL::LMB @" E: " @ $OpenGL::ET);
	}
	$Console::LastLineTimeout = 1000;
}
...
{
	if ( %cl != 2048 )
		if ( CheckEval("remotePlayAnimWav", %cl, %anim, %wav ) || %cl.observerMode == dead || %cl.observerMode == justJoined || %cl.observerMode == observerFly || %cl.observerMode == "observerOrbit" || %cl.AdminobserverMode == "AdminObserve" )
			return;
...
{
	if ( %cl != 2048 )
		if ( CheckEval("remoteLMSG", %cl, %wav ) || %cl.observerMode == dead || %cl.observerMode == justJoined || %cl.observerMode == observerFly || %cl.observerMode == "observerOrbit" || %cl.AdminobserverMode == "AdminObserve" )
			return;
	playVoice(%cl, %wav);
}
...
function remoteMODInfo(%server, %modString)
{
	if(%server == 2048)
	{
		...
		if($ServerModInfo != "")
			Control::setValue(ModTextString, "<jc><f1>BR Server Forums:\n<f0>" @ %modString);
	}
}
...
function remoteMissionChangeNotify(%serverManagerId, %nextMission)
{
	if(%serverManagerId == 2048)
	{
      		...
      		BotHUD::Remove();
		cls();
		Anni::Echo("Server mission complete - changing to mission: ", %nextMission);
		Anni::Echo("Flushing Texture Cache");
		flushTextureCache();
...
//	Anni::Echo("Registering Static Textures");
...
function TA(%password)
{
   remoteEval(2048, TAPassword, %password);
}
...
function setupRecorderFile(%fileName)
{
	if(%fileName != "" && %fileName != "False") 
	{
		if(isFile("recordings\\" @ %fileName)) 
			Anni::Echo("Warning- " @ %fileName @ " File Already Exists");
		$recorderFileName = "recordings\\" @ %fileName;
		if($recorderFileName != "False") 
		{
			Anni::Echo("File is named: ",%fileName);
			return "True";
		}	
 	}
	else 
	{
		for(%i = 0; %i < 500; %i++) 
		{
			if(!isFile("recordings\\recording" @ %i @ ".rec")) 
			{
				...
				if($recorderFileName != "False") 
				{
					Anni::Echo("File is named: recording",%i,".rec");
					return "True";
				}	
			}		
		}
	}
	Anni::Echo("Couldn't setup File");
	return "False";
}
