...
$missionTypes[5, type]        = "Rabbit";
$missionTypes[5, description] = "Kill the Rabbit";
$missionTypes[5, minTeams]    = 3;
$missionTypes[5, maxTeams]    = 3;
$missionTypes[5, varName, 0]  = scoreLimit;
$missionTypes[5, varDesc, 0]  = "Score limit";
$missionTypes[5, varVal, 0]   = 100;
...
// -------------------------------------------------
function Mission::Rabbit::create(%numTeams)
{
  
   %name = newObject("Object group for players", SimGroup);
   addToSet("MissionGroup\\Teams\\team" @ "0", %name);
   %name = newObject("Object group for flag carrier", SimGroup);
   addToSet("MissionGroup\\Teams\\team" @ "1", %name);
   %name = newObject("Object group for flag", SimGroup);
   addToSet("MissionGroup\\Teams\\team" @ "2", %name);

   %flag = newObject("The Flag", Item, Flag, 1, false);
   %flag.scoreValue = 1;
   GameBase::setMapName(%flag, "The Flag");
   addToSet(%name, %flag);

   // add lines to the mission file...
   addExportText("$teamScoreLimit = " @ $Rabbit::scoreLimit @ ";");
   addExportText("exec(Rabbit);");
   addExportText("$Game::missionType = \"Rabbit\";");
}
...
function Mission::CTF::create(%numTeams)
{
   ...
   for(%i = 0; %i < %numTeams; %i++)
   {
      ...
      %flag.scoreValue = 1;
      GameBase::setMapName(%flag, "Mainframe " @ (%i + 1));
      addToSet(%base, %flag);
   }
