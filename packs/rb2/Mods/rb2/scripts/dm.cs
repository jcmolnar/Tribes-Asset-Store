...
{	  
   ...
   Player::setItemCount(%clientId,FlareGun,1);
   Player::setItemCount(%clientId,EagleAmmo,50);
   // Player::setItemCount(%clientId,Handgrenade,2);
   Player::setItemCount(%clientId,AmmoPack,1);
   	Player::useItem(%pl,AmmoPack);

// --------------------------------------BEGIN RANDOM WEAPON GIVING-------------------------
//Deadtaco's code to spawn you with a random primary weapon, secondary, and grenade after every spawn

//random primary weapon
%rnd = floor(getRandom() * 18);
	if (%rnd == 0) 
		%rndweapon = lavacannon;
	if (%rnd == 1) 
		%rndweapon = chaingun;
	if (%rnd == 2) 
		%rndweapon = plasmagun;
	if (%rnd == 3) 
		%rndweapon = disclauncher;
	if (%rnd == 4) 
		%rndweapon = grenadelauncher;
	if (%rnd == 5) 
		%rndweapon = famas;
	if (%rnd == 6) 
		%rndweapon = decoygun;
	...
	if (%rnd == 8) 
		%rndweapon = remington;
	if (%rnd == 9) 
		%rndweapon = autoshotgun;
	if (%rnd == 10) 
		%rndweapon = britmag;
	if (%rnd == 11) 
		%rndweapon = oicw;
	if (%rnd == 12) 
		%rndweapon = klashnikov;
	if (%rnd == 13) 
		%rndweapon = berettaSMG;
	if (%rnd == 14) 
		%rndweapon = famas;
	if (%rnd == 15) 
		%rndweapon = sniperrifle;
	if (%rnd == 16) 
		%rndweapon = saw;
	if (%rnd == 17) 
		%rndweapon = sniperrifle;
	if (%rnd == 18) 
		%rndweapon = ingram;
Player::setItemCount(%clientId,%rndweapon,1);
Player::setItemCount(%clientId,$WeaponAmmo[%rndweapon],100);
...
%rnd = floor(getRandom() * 7);
	...
	if (%rnd == 5) 
		%rndsecond = blaster;
	if (%rnd == 6) 
		%rndsecond = plasmacannon;
	if (%rnd == 7) 
		%rndsecond = plasmacannon;

Player::setItemCount(%clientId,%rndsecond,1);
Player::setItemCount(%clientId,$WeaponAmmo[%rndsecond],100);

//random grenades

%rnd = floor(getRandom() * 2);
	if (%rnd == 0) 
		%rndgrenade = OriginalGrenade;
	if (%rnd == 1) 
		%rndgrenade = OriginalGrenade;
	if (%rnd == 2) 
		%rndgrenade = OriginalGrenade;

Player::setItemCount(%clientId,grenade,2);
Player::setItemCount(%clientId,%rndgrenade,2);
Player::useItem(%pl, %rndweapon);

bottomprint(%clientId, "<jc><f1>You have spawned in with a " @ %rndweapon.description @ ", a " @ %rndsecond.description @ " and a " @ %rndgrenade.description, 5);		 


// --------------------------------------END RANDOM WEAPON GIVING-------------------------
