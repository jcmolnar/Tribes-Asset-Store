...
function remoteMODInfo(%server, %modString)
{
   if(%server == 2048)
   {
      ...
      if($ServerModInfo != "")
      {
         Control::setValue(ModTextString, "<jc><f1>NOTICE: This is not your momma's game\n<f0>" @ %modString);
      }
   }
}
...
{
   if(%pctDone < 0.1)
      %text = "Use EXTRA AMMO CLIPS to reload empty guns!";
   else if(%pctDone < 0.2)
      %text = "Buy OICW Grenades for your OICW!";
   else if(%pctDone < 0.3)
      %text = "The BARRETT is a great sniper rifle!";
   else if(%pctDone < 0.4)
      %text = "Deploy ramps and springboards for Heavier Units to get to high places!";
   else if(%pctDone < 0.5)
      %text = "GO BLOW SOMEONE UP!";
   else if(%pctDone < 0.6)
