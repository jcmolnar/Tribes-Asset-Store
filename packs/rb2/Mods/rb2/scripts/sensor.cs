...
function Sensor::onDamage(%this,%type,%value,%pos,%vec,%mom,%object)
{
	...
	if(GameBase::getTeam(%this) == GameBase::getTeam(%object))
	{
		...
		if(%name != DeployableMotionSensor && %name != DeployablePulseSensor && %name != DeployableSensorJammer && %name != DeployableLRMotionSensorPack )				
...
SensorData DeployableLongRangeMotionSensor
{
	...
	className = "DeployableSensor";
	shapeFile = "radar";    // "sensor_pulse_med";
	shadowDetailMask = 16;
	visibleToSensor = true;
...
function DeployableLongRangeMotionSensor::onDestroyed(%this)
{
	$TeamItemCount[GameBase::getTeam(%this) @ "DeployableLongRangeMotionSensor"]--;
