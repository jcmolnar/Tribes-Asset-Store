...
// The role of this function is primarily just to be a educational reference for scripters. (But it does work.)
...
function JetToPos(%client, %targetPos)
{
	if (Player::getArmor(%client) == False) // If the client is dead then clean up here and quit looping on it.
	{
		...
		echo(%name @ " died. Stopping JetToPos.");
	...
	if ($JetToPos[%client] == "")
		$JetToPosTime[%client] = 0;
	$JetToPosTime[%client]++;
	if ($JetToPosTime[%client] > 1500) // If too much time has been spent on this then clean up here and quit looping on it.
	{
		$JetToPos[%client] = ""; // Clean up dead entries.
		%name = Client::getName(%client);
		echo("Max. JetToPos time reached for " @%name @ ". Stopping JetToPos.");
		$JetToPosDelay[%client] = 2;
		return False;  // Bail out.
		break;
	}
...
// Finite Vertical Navigation.
	...
	if(%speedX >= 100 || %speedY >= 100 || %speedZ >= 100) // Error Trap.
	{
 ...
 // Don't make the client go "FWING!!!" (In any direction.) :P :)
	if(%dist >= 25) // Coarse Flight.
	{
		if(%speedX <=  -10)
			if(%destPosX <= -1)
				%destPosX = -0.1; //%destPosX*0.010;
		if(%speedY <= -10)
			if(%destPosY <= -1)
				%destPosY = -0.1; //%destPosY*0.010;
		if(%speedZ <= -15)
			if(%destPosZ <= -1)
				%destPosZ = %destPosZ*0.010;
		if(%speedZ <= -0.000015)
			if(%destPosZ <= -0.000015)
				%destPosZ = %destPosZ+Vector::neg(%destPosZ);
		if(%speedX >= 10)
			if(%destPosX >= 1)
				%destPosX = %destPosX*-0.010; //0.1;
		if(%speedY >= 10)
			if(%destPosY >= 1)
				%destPosY = %destPosY*-0.010; //0.1;
		if(%speedZ >= 15)
			if(%destPosZ >= 1)
				%destPosZ = %destPosZ*-0.010;
		if(%speedZ >= 0.000015)
		...
		if(%speedY <= -15)
			%destPosY = %brakeY;
		if(%speedZ <= -0.1)
			Player::ApplyImpulse(%client, "0 0 "@%parachuteVal);
		if(%speedX >= 15)
			%destPosX = %brakeX;
		if(%speedY >= 15)
			%destPosY = %brakeY;
		if(%speedZ >= 25)
			Player::ApplyImpulse(%client, "0 0 "@%vertAdjust); //parachuteVal);
//		if(%speedZ <= 1)
//			Player::ApplyImpulse(%client, "0 0 "@%vertAdjust); //parachuteVal); 
	}
	if(%dist <= 25) // The Alignment.
	{
		if(%dist >= 10)
		{
			if(%speedX <=  -10)
				if(%destPosX <= -5)
					%destPosX = %destPosX*0.010;
			if(%speedY <= -10)
				if(%destPosY <= -5)
					%destPosY = %destPosY*0.010;
			if(%speedZ <= -10)
				if(%destPosZ <= -5)
					%destPosZ = %destPosZ*0.010;
			if(%speedX >= 10)
			...
			if(%speedZ <= -0.1)
				Player::ApplyImpulse(%client, "0 0 "@%parachuteVal);
			Player::ApplyImpulse(%client, "0 0 "@%vertAdjust);
//			if(%dist <= 20) // A higher value for faster/sloppier destinations.
//				Player::ApplyImpulse(%client, "0 0 "@%parachuteVal);
		}
	}
	if(%dist <= 10) // The Landing.
	{
			if(%speedX <=  -5)
				if(%destPosX <= -1)
					%destPosX = %destPosX*0.10;
			if(%speedY <= -5)
				if(%destPosY <= -1)
					%destPosY = %destPosY*0.10;
			if(%speedZ <= -5)
				if(%destPosZ <= -1)
					%destPosZ = %destPosZ*0.10;
			if(%speedX >= 5)
				if(%destPosX >= 1)
					%destPosX = %destPosX*-0.10;
			if(%speedY >= 5)
	...
	if(%lockY == true)
		%destPosY = %brakeY;
// Are we stuck? Hmm, lets try to fix it. (For field obstacles.)
	if(%dist >= 10)
	{
		if(%speedY <= 0.1)
			if(%speedY >= 0)
				if(%lockX != true)
					{
						%destPosX = %reverseX*0.001;
						%stuck = true;
						%freeUp = "+Y axis. Reversing X to "@%reverseX;
					}
		if(%speedY >= -0.1)
			if(%speedY <= 0)
				if(%lockX != true)
		...
		if(%speedX <= 0.1)
			if(%speedX >= 0)
				if(%lockY != true)
				{
					%destPosY = %reverseY*0.001;
					%stuck = true;
					%freeUp = "+X axis. Reversing Y to "@%reverseY;
				}
		if(%speedX >= -0.1)
			if(%speedX <= 0)
				if(%lockY != true)
		...
		if(%speedZ <= 0.1)
			if(%speedZ >= 0)
			{
				%destPosZ = %reverseZ;
				%stuck = true;
				%freeUp = "+Z axis. Reversing "@%reverseZ;
...
// Apply the results.	
	...
	{
		if($JetToPos != "break") // Override input. (Just in case.)
		{
			if(%destPos != "")
			{
				//Player::ApplyImpulse(%client,"0 0 "@%vertAdjust*10);
				Player::ApplyImpulse(%client, %destPos);
				schedule("JetToPos("@%client@");",0.01); // Comment out this line if if you're gonna call the function from elsewhere.
				// Note: You'll need your external function call to JetToPos repeatedly untill goal is met.
	...
	{
		if($JetToPos == show)
			echo(%name@" made it to the destination!!! WooHoo!!! :)");
		$JetToPos[%client] = ""; // We're all done with them.
	}
}

function setJetToPos(%client)
{
	$setTargDest = GameBase::getPosition(%client);
	echo("Position = "@$setTargDest);
}

function go(%client)
{
	JetToPos(%client,$setTargDest);
}
