...
{
	// make sure we're not still bugged
   $MessagesBugged[0,1] = 0;
   $MessagesBugged[0,2] = 0;
   $MessagesBugged[0,3] = 0;
   $MessagesBugged[0,4] = 0;
   $MessagesBugged[1,0] = 0;
   $MessagesBugged[1,2] = 0;
   $MessagesBugged[1,3] = 0;
   $MessagesBugged[1,4] = 0;
   $MessagesBugged[2,1] = 0;
   $MessagesBugged[2,0] = 0;
   $MessagesBugged[2,3] = 0;
   $MessagesBugged[2,4] = 0;
...
function ObjectiveMission::setObjectiveHeading() //Not sure where this came from - I believe it is shifter or Nimmod.  Borrowed to save time
{
	...
	if($missionComplete == "True") 
	{
		...
		for(%i = -1; %i < getNumTeams(); %i++)
			...
			Team::setObjective(%i, %lineNum++, "<L14><f8>- Reality Bites - Mission Summary");
			...
			// =================Write scores to file first -- DEADTACO Reality Bites =======================
			%index = 0; 
			%c = 0;
			while(%index < %numClients)
			{
				$ScoreRecord[%c] = "," @ Client::getName(%clientList[%c]) @ ",SCORE:,"  @ (%clientList[%c]).score @ ",KILLS:," @ (%clientList[%c]).ScoreKills;
				%index++;
				%c++;
			} 

			$ScoreRecord[%numClients] = " ";
			%nummy = 100;     // floor(getRandom() * (25000))
			if ($FileWritten != 1)  //For some reason, scores are added 3 times in a row.  This makes sure it only happens once.
				{
				export("$ScoreRecord*", "config\\RBscores_Tally.txt", true);
				$FileWritten = 1;
				}
...
Team::setObjective(%i, %lineNum++, "<L2><f2>Rank\t\t\t\t\t\t<f0>Player\t\t\t\t\t\t<f2>Score\t\t\t\t\t\tKills");


			//========================================= Sort Client List By Score
			...
			while(%index < %numClients)
			{
				...
				if($debug) echo ("*** Printing Player - " @ Client::getName(%clientList[%c]) @ " Stats" @ (%clientlist[%c]));
				
Team::setObjective(%i, %lineNum++, "<L2><f2>#" @ (%c + 1) @ " \t\t\t\t\t\t<f0>" @ Client::getName(%clientList[%c]) @ "\t\t\t\t\t<f2>"  @ (%clientList[%c]).score @ "\t\t\t\t\t<f0>" @ (%clientList[%c]).ScoreKills);

				//Team::setObjective(%i, %lineNum++, "<L14><f0>Life Time Scores:");
				//Team::setObjective(%i, %lineNum++, "<L14><f0>Kills= " @ (%clientList[%c]).TotalKills @ "     Deaths= " @ (%clientList[%c]).TotalDeaths @ "     TeamKills= " @ (%clientList[%c]).TotalTKCount @ "     Mainframe Caps = " @ (%clientList[%c]).TotalFlags @ "    Missions Completed - " @ %clientList[%c].MissionComplete @ " With A Total Score Of " @ %clientList[%c].TotalScore @ ".");
	...
	else
	{
		for(%i = -1; %i < getNumTeams(); %i++)
		{
			...
			Team::setObjective(%i, %lineNum++, "<L2><f2>Rank\t\t\t\t\t\t<f0>Fighter\t\t\t\t\t\t<f2>Score\t\t\t\t\t\t<f0>Kills");
...
{
   %client = Player::getClient(%this); //RSP
   ...
   for(%i = 0; (%obj = Group::getObject(%set, %i)) != -1; %i++)
	...
	echo(Player::getClient(%this) @ " - Entered the mission area");

	Client::sendMessage(Player::getClient(%this),1,"You have returned to the mission area.");
}
...
function Player::leaveMissionArea(%this)
{
	...
	%this.outArea=1;
	if ($rb::rebound != true)
	    Client::sendMessage(Player::getClient(%PLAYER),1,"You left the mission area.  Get back there.");

	alertPlayer(%this, 3);

	echo(Player::getClient(%this) @ " - Left the mission area"); //RSP

	oobPlayer(%client,10,%this); //RSP
}
...
function alertPlayer(%player, %count)
{
	if(%player.outArea == 1) {
		if(%count > 0) {
			...
			if ($rb::rebound == true)
			{
			Client::sendMessage(Player::getClient(%PLAYER),2,"You were spit back into the battle.");
			%myVelocity = Item::getVelocity(%player);
			%Xvel = GetWord(%myVelocity, 0) * -1.0;
			%yvel = GetWord(%myVelocity, 1) * -1.0;
...
// oobPlayer func added for RSP MOD by HightechRedneck
function oobPlayer(%client,%time,%player)
{
	%clientName = client::GetName(%client);

	if(%player.outArea && !Player::isDead(%player) && $rb::rebound != true) {
		if(%time == 10){
			Client::sendMessage(%client, 1, "You have " @ %time @ " seconds to return to the mission area before you start to die!");
			schedule("oobPlayer(" @ %client @ ", " @ %time - 1 @ ", " @ %player @ ");",1,%player);
		}
		else if(%time == 5) {
			Client::sendMessage(%client, 1, "You have 5 seconds to return to the mission area before you start to die!");
			schedule("oobPlayer(" @ %client @ ", " @ %time - 1 @ ", " @ %player @ ");",1,%player);
		}
		else if(%time == 0) {
			Client::sendMessage(%client, 1, "You were warned!");
			
			leaveMissionAreaDamage(%client);
			
			schedule("oobPlayer(" @ %client @ ", -1, " @ %player @ ");",1,%player);
		}
		else if(%time == -1) {
			leaveMissionAreaDamage(%client);
...
function leaveMissionAreaDamage(%client) // Needed for RSP copied from dm.cs
{
...
function TowerSwitch::onCollision(%this, %object)
{
	...
	if(%oldTeam == -1)
	{
		...
		echo("ADMINMSG: **** " @ %touchClientName @ " claimed the " @ %this.objectiveName @ " for the " @ getTeamName(%playerTeam) @ " team!");
	}
	else
	{
		if(%this.objectiveLine)
		{
			...
			echo("ADMINMSG: **** " @ %touchClientName @ " claimed the " @ %this.objectiveName @ " from the " @ getTeamName(%oldTeam) @ " team!");
			%this.numSwitchTeams++;	
...
{
	//give player a point for capturing tower!
	if(%this.numSwitchTeams == %numChange)
	{
		%client.score+=5;
		Game::refreshClientScore(%client);
		Client::sendMessage(%client, 0, "You receive five points for holding your captured tower!");
	}
}
...
function Flag::getObjectiveString(%this, %forTeam)
{
   ...
   if($missionComplete)
   {
      ...
      {
         if(%thisTeam == %forTeam)
            return "<Bflag_atbase.bmp>\nYour mainframe was captured " @ %this.enemyCaps @ " times.";
         else
            return "<Bflag_enemycaptured.bmp>\nYour team captured the " @ getTeamName(%thisTeam) @ " mainframe " @ %this.caps[%forTeam] @ " times.";
      }
  		else 
      	return "<Bflag_atbase.bmp>\nThe " @ getTeamName(%thisTeam) @ "'s mainframe was captured " @ %this.enemyCaps @ " times.";
   }
   else
   {
      if(%thisTeam == -1)
			...
			if(%forTeam != -1) {
         	...
         	else if(%this.carrier != -1)
         	{
         	   if(GameBase::getTeam(%this.carrier) == %forTeam)
         	      return "<Bflag_atbase.bmp>\nConvey " @ %this.objectiveName @ " to an empty mainframe stand. (carried by " @ Client::getName(Player::getClient(%this.carrier)) @ ")";
         	   else
         	      return "<Bflag_enemycaptured.bmp>\nWaylay " @ Client::getName(Player::getClient(%this.carrier)) @ " and convey " @ %this.objectiveName @ " to your base.";
         	}
         	else if(%this.atHome)
         	   return "<Bflag_neutral.bmp>\nGrab " @ %this.objectiveName @ " and convey it to an empty mainframe stand.";
         	else
         	   return "<Bflag_notatbase.bmp>\nFind " @ %this.objectiveName @ " and convey it to an empty mainframe stand.";
      	}
			else {
      ...
      else
      {
         ...
         {
            if(%this.atHome)
               return "<Bflag_atbase.bmp>\nDefend your mainframe to prevent enemy captures.";
            else if(%this.carrier != -1)
               return "<Bflag_enemycaptured.bmp>\nReturn your mainframe to base. (carried by " @ Client::getName(Player::getClient(%this.carrier)) @ ")";
            else
               return "<Bflag_notatbase.bmp>\nReturn your mainframe to base. (dropped in the field)";
         }
         else
         {
				if(%forTeam != -1) {
            	if(%this.atHome)
            	   return "<Bflag_enemycaptured.bmp>\nGrab the " @ getTeamName(%thisTeam) @ " mainframe and touch it to your's to score " @ %this.scoreValue @ " points.";
            	else if(%this.carrier == -1)
            	   return "<Bflag_notatbase.bmp>\nFind the " @ getTeamName(%thisTeam) @ " mainframe and touch it to your's to score " @ %this.scoreValue @ " points.";
            	   ...
            	   return "<Bflag_enemycaptured.bmp>\nWaylay enemy carrier " @ Client::getName(Player::getClient(%this.carrier)) @ " and steal his mainframe.";
         	}
				else {
            	if(%this.atHome)
            	   return "<Bflag_atbase.bmp>\nThe " @ getTeamName(%thisTeam) @ " mainframe is at their base.";
            	else if(%this.carrier == -1)
            	   return "<Bflag_notatbase.bmp>\nThe " @ getTeamName(%thisTeam) @ " mainframe has been dropped in the field.";
            	else 
            	   return "<Bflag_atbase.bmp>\n" @ Client::getName(Player::getClient(%this.carrier)) @ " has the " @ getTeamName(%thisTeam) @ " mainframe.";
				}
			}         
      }
   }
...
function Flag::onDrop(%player, %type)
{
	...
	%homepos = ($teamFlag[%flagteam]).originalPosition; //========= Get Base location for live player.
	...
	if(%flagTeam == -1)
	{
		...
		echo("ADMINMSG: **** " @ %dropClientName @ " dropped the " @ getTeamName(%flagTeam) @ " mainframe!");
		
	}
	else
	{
		MessageAllExcept(%playerClient, 0, %dropClientName @ " dropped the " @ getTeamName(%flagTeam) @ " mainframe!");
		Client::sendMessage(%playerClient, 0, "You dropped the " @ getTeamName(%flagTeam) @ " mainframe!");
		if(%flagteam != %playerteam && %disthome <= 100)
		{
			%playerClient.score -= 1; // ****
 			Game::refreshClientScore(%playerClient); // ****
			MessageAll(0, Client::getName(%playerClient) @ " loses a point for dropping the mainframe and almost breaking it."); // ****
		}
		TeamMessages(1, %flagTeam, "Your mainframe was lost in the field.", -2, "", "The " @ getTeamName(%flagTeam) @ " mainframe was dropped in the field.");
	...
	schedule("Flag::AIReturnFlag(" @ %flag @ ");", 4);  //by Werewolf. Makes the nearest AI return the flag.
		...
		else
		{
			...
			if(%flagTeam == -1)
			{
				...
				else
				{
					...
					TeamMessages(0, %holdTeam, "Your mainframe was returned to base.~wflagreturn.wav", -2, "", "The " @ getTeamName(GameBase::getTeam(%flag.flagStand)) @ " mainframe was returned to base.~wflagreturn.wav");
			...
			{
				TeamMessages(0, %flagTeam, "Your mainframe was returned to base.~wflagreturn.wav", -2, "", "The " @ getTeamName(%flagTeam) @ " mainframe was returned to base.~wflagreturn.wav");
				GameBase::setPosition(%flag, %flag.originalPosition);
...
//	echo("Flag collision ", %object);
	...
	if(%flagTeam == %playerTeam)
	{
		...
		if(!%this.atHome)
		{
			...
			%this.atHome = true;
			MessageAllExcept(%playerClient, 0, %touchClientName @ " returned the " @ getTeamName(%playerTeam) @ " mainframe!~wflagreturn.wav");
			Client::sendMessage(%playerClient, 0, "You returned the " @ getTeamName(%playerTeam) @ " mainframe!~wflagreturn.wav");
			teamMessages(1, %playerTeam, "Your mainframe was returned to base.", -2, "", "The " @ getTeamName(%playerTeam) @ " mainframe was returned to base.");
			%playerpos = gamebase::getPosition(%object);	
			if(Vector::getDistance(%this.originalPosition,%playerpos) > 1)
			{
				%playerClient.score += 1; 
				echo("%playerClient  " @ %playerClient);
				echo("Gaining POINTS, %PLAYERCLIENT.SCORE = " @ %playerClient.score);
				Game::refreshClientScore(%playerClient);
				messageAll(0, Client::getName(%playerClient) @ " receives a point for returning his mainframe.");
			}
			
			%this.pickupSequence++;
			ObjectiveMission::ObjectiveChanged(%this);
		}
		else // if(!Player::isAIControlled(%object))                Modified by Werewolf, so that AI cannot capture flags but RETURN them. Not any more !
		{
			...
			if(%object.carryFlag != "")
			{
				...
				if(%enemyTeam != -1)
				{
					MessageAllExcept(%playerClient, 0, %touchClientName @ " stole the " @ getTeamName(%enemyTeam) @ " mainframe!~wflagcapture.wav");
					Client::sendMessage(%playerClient, 0, "You stole data from the " @ getTeamName(%enemyTeam) @ " mainframe!~wflagcapture.wav");
					TeamMessages(1, %playerTeam, "Your team stole the mainframe's data.", %enemyTeam, "Your team's mainframe was stolen and hacked.");
					echo("ADMINMSG: **** " @ %touchClientName @ " captured the " @ getTeamName(%enemyTeam) @ " mainframe!~wflagcapture.wav");            
					...
					//flag carrier gets 10 points for caputure
					%playerClient.score += $CapPoints;
					Game::refreshClientScore(%playerClient);
					messageAll(0, Client::getName(%playerClient) @ " receives a " @ $CapPoints @ " point capture bonus.");
	...
	else
	{
		...
		//Initial Truce Flag Kill
		%time = $Server::InitialTruce - ((getSimTime() - $missionStartTime)/ 60);


		%shooterObj = Client::getControlObject(%playerclient);
		// messageall(1, %playerclient @ " recharges at " @ GameBase::getRechargeRate(%shooterobj) @ " in armor " @ player::getarmor(%playerclient));

		if((player::getarmor(%playerclient) == "hlarmor" || player::getarmor(%playerclient) == "hlfemale") && GameBase::getRechargeRate(%shooterobj) >=14)
		{
			centerprint(%playerClient,"<jc>YOUR ARMOR IS TOO LIGHT TO HOLD A MAINFRAME WHILE OVERCHARGED.",3);
			// GameBase::applyDamage(%object,$DebrisDamageType,10,GameBase::getPosition(%object),"0 0 0","0 0 0",%this);
			return;
		}


		if((%time > 0) && $Server::Truce)
		{
			centerprint(%playerClient,"<jc>You have violated the truce, but nobody cares.",3);
			// GameBase::applyDamage(%object,$DebrisDamageType,10,GameBase::getPosition(%object),"0 0 0","0 0 0",%this);
			return;
		}
		...
		if(%object.carryFlag == "")
		{
			if(%object.outArea == "")
			{
				...
				if(%flagTeam != -1)
				{
					MessageAllExcept(%playerClient, 0, %touchClientName @ " took the " @ getTeamName(%flagTeam) @ " mainframe! ~wflag1.wav");
					Client::sendMessage(%playerClient, 0, "You took the " @ getTeamName(%flagTeam) @ " mainframe! ~wflag1.wav");
					TeamMessages(1, %playerTeam, "Your team has the " @ getTeamName(%flagTeam) @ " mainframe.", %flagTeam, "Your team's mainframe has been taken.");
					echo("ADMINMSG: **** " @ %touchClientName @ " took the " @ getTeamName(%flagTeam) @ " mainframe!");
				}
				else
				{
					...
					if(%hteam != -1)
					{
						...
						echo("ADMINMSG: **** " @ %touchClientName @ " took the " @ getTeamName(%hteam) @ " mainframe!");						%this.holdingTeam = -1;
						%this.holder.flag = "";
					}
					else
					{
						...
						echo("ADMINMSG: **** " @ %touchClientName @ " took the " @ %this.objectiveName @ " mainframe!");						echo("ADMINMSG: **** " @ %touchClientName @ " captured the " @ %this.objectiveName @ " mainframe!~wflagcapture.wav");
					}
				}
				%this.trainingObjectiveComplete = true;
				ObjectiveMission::ObjectiveChanged(%this);

				//Now we'll set up all enemy AI players to chase the flag carrier. by Werewolf
				...
				Client::sendMessage(%playerClient, 1, "Mainframe not in mission area.");
		}
	}
}
   ...
   %team = Client::getTeam(%client);
	...
	{ 
		for(%s = $teamFlagStand[%team]; %s != ""; %s = %s.nextFlagStand) 
		{
			if(%s.flag == "") {
				...
				%posX = getWord(%pos,0);
				%posY = getWord(%pos,1);
   	   	issueCommand(%client, %client, 0,"Take " @ %flag.objectiveName @ " to empty mainframe plug.~wcapobj", %posX, %posY);
				return;
			}
	        }
	}
	else
	{
		...
		%posY = getWord(%pos,1);
  	   	...
  	   	issueCommand(%client, %client, 0,"Take the " @ getTeamName(%flagTeam) @ " mainframe to our mainframe.~wcapobj", %posX, %posY);
				// Stop other team members.
   ...
   if(%killerTeam == %flagTeam)
   {
      ...
      if(%this.carrier == -1)
      {
         ...
         if(Vector::getDistance(%flagPos, %playerPos) < 80)
         {
            %killerId.score++;
            Game::refreshClientScore(%killerId);
            messageAll(0, Client::getName(%killerId) @ " gets a bonus for defending the mainframe!");
         }
      }
   }
   else
      ...
      if(%this.carrier != -1)
      {
         ...
         if(%carrierTeam == %killerTeam)
         {
               ...
               GameBase::getPosition(%killer)) < 80)
            {
               %killerId.score++;
               Game::refreshClientScore(%killerId);
               messageAll(0, Client::getName(%killerId) @ " gets a bonus for defending the mainframe carrier!");
            }               
         }
      }
   }
   ...
   if(%this.carrier == %playerId)
   {
      ...
      Player::setItemCount(%playerId, "Flag", 0);
   	...
   	else
   	{
			...
			MessageAllExcept(%playerClient, 0, %clientName @ " left the mission area while carrying the " @ getTeamName(%team) @ " mainframe!");
      	Client::sendMessage(%playerClient, 0, "You left the mission area while carrying the " @ getTeamName(%team) @ " mainframe!");
      	TeamMessages(1, %team, "Your mainframe was returned to base.~wflagreturn.wav", -2, "", "The " @ getTeamName(%team) @ " mainframe was returned to base.");
