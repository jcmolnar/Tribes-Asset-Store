...
TurretData ArbitorBeacon
{
	...
	activationSound = SoundRemoteTurretOn;
	deactivateSound = SoundRemoteTurretOff;
	// PORT NOTE (2026-09-12): was "explosionId = defaultDebrisSmall;". That is a
	// DebrisData block assigned to an Explosion field, so setDataTypeExplosionBlock
	// (FearDynamicDataPlugin.cpp:1459) logged "Explosion data block
	// \"defaultDebrisSmall\" is undefined" and set the field to -1. Commenting the
	// line out leaves explosionId at exactly that -1, so the Arbitor Beacon behaves
	// identically and the boot log is clean. GreyFlcn hit the same line in v1G and
	// spelled it debrisId; not copied here, because this block already sets
	// debrisId = flashDebrisMedium above and overriding it WOULD change behaviour.
