...
{ 
	...
	minEnergy = 0.1;
	maxEnergy = 0.1;
	// PORT NOTE (2026-09-12): no BoosterAmmo datablock exists -- not in this release,
	// not in v1G, not in base. The engine logged "Item data block BoosterAmmo
	// undefined" and left ammoType at -1; the booster runs on minEnergy/maxEnergy
	// above, not on ammo. Commented out, which is exactly what GreyFlcn did in v1G
	// (item.cs:4998, 5039). Behaviour is unchanged; the boot log is clean.
