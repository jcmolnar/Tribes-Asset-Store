...
// host menu wins.
if($Server::HostName == "")
	$Server::HostName = "Shifter Server";		// The name of your
...
// port and a real password before uncommenting these two lines.
...
//========================================================= Mission Settings
if($pref::LastMission == "")
	$pref::LastMission = "Broadside";	//== Starting Mission if Random Start is off  (seed-only, see note at top)
...
// PORT NOTE (2026-09-12): commented out with the master block below. The XLMaster
// list IS still read on this client (FearCSDelegate.cpp), and every address here is
// a raw 1999 IP that now belongs to someone else entirely.
...
// PORT NOTE (2026-09-12): the master-server block is commented out, all of it.
// Every host below is a Dynamix machine that has been dead for two decades, and
// assigning these would replace the master list the Modern Client resolves at boot
// -- which is how a Shifter server stops appearing in anyone's browser. Leave the
// client's own list alone.
