...
function BotFuncs::StartFire(%aiId)
{
  ...
  $FireOK[%aiId]=false;
  $FireCycle[%aiId]=1;
  BotFuncs::KeepFiring(%aiId);
}

function BotFuncs::KeepFiring(%aiId)
{
  $FireCycle[%aiId]*=-1;

  if ($FireCycle[%aiId]==-1)
    postAction(%aiId, IDACTION_FIRE1, 0);
  else
    postAction(%aiId, IDACTION_BREAK1, 0);

  if ($FireOK[%aiId])
    schedule("BotFuncs::KeepFiring("@%aiId@");",0.2);
}


function BotFuncs::StopFire(%aiId)
{
  $FireOK[%aiId]=false;
}
