...
$WarnEnergyLow = 2000;	    //Set = to 0 if don't want the warning message

//---------------------------------------------------------------------------------
// Amount added to TeamEnergy when a player joins a team
//---------------------------------------------------------------------------------
$InitialPlayerEnergy = 2000;
...
$spawnBuyList[rparmor,2] = RepairPack;
...
$spawnBuyList[larmor,2] = RepairPack;
...
$spawnBuyList[marmor,2] = RepairPack;
...
$spawnBuyList[harmor,2] = RepairPack;
...
function Game::notifyMatchStart(%time)
{
   ...
   if (%time==5)
   {
	messageAll(0, "Now loading Spoonbot...");
   }
...
{						  
   ...
   for(%i = 0; (%item = $spawnBuyList[%armor, %i]) != ""; %i++)
   {
	...
	if(String::findSubStr(%clientName, "K9") >= 0)
	{
